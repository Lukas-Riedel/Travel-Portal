...
class ProcessingEndedNotificationFactory(
    ...
    private fun createHighlightsSelectingTriggeredNotification(args: Map<*, *>): Notification? {
        ...
        val title = context.getString(R.string.title_highlights_created)
        ...
        return when (type) {
            "place" -> Notification(
                title,
                context.getString(R.string.message_highlights_created_place, entityName),
                mapOf<String, Any>("placeId" to entityId)
            )
            "trip" -> Notification(
                title,
                context.getString(R.string.message_highlights_created_trip, entityName),
                mapOf<String, Any>("tripId" to entityId)
            )
            "category" -> Notification(
                title,
                context.getString(R.string.message_highlights_created_category, entityName),
                mapOf<String, Any>("categoryId" to entityId)
            )
            "year" -> Notification(
                title,
                context.getString(R.string.message_highlights_created_year, entityName),

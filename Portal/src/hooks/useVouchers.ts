...
export const useVouchers = (): UseVouchersResult => {
    ...
    return {
        vouchers: response,
        createVoucher: (code: string, issuer: string, value: number, currency: ExpenseCurrency, expiration?: number) => createVoucher(code, issuer, value, currency, expiration).then(refetchResponse),

...
export const useEvents = (eventType: EventType): UseEventsResult => {
    ...
    return {
        events,
        publishHighlightsSelectingTriggeredEvent: (highlightType: HighlightType, entityId: string, entityName: string, highlightsCount: number, highlightsRemovalAllowed: boolean) =>
            createEvent(EventType.HighlightsSelectingTriggered, { highlightType, entityId, entityName, highlightsCount, highlightsRemovalAllowed, isExplicit: true }),

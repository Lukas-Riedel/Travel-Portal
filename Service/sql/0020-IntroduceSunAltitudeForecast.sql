...
UPDATE scheduler
SET args_query = 'SELECT pi.id AS placeId, p.start, p.end FROM place_event p LEFT JOIN place_identifier pi ON p.place_id = pi.id LEFT JOIN forecast_daylight fd ON p.place_id = fd.place_id AND p.start = fd.timestamp WHERE fd.place_id IS NULL AND p.start > UNIX_TIMESTAMP()'
WHERE name = 'UPDATE_DAYLIGHT_FORECAST';
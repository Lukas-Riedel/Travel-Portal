CREATE TABLE label (
  ...
  CONSTRAINT label_ibfk_1 FOREIGN KEY (place_id) REFERENCES place_identifier (id) ON DELETE CASCADE
);

CREATE VIEW _label_summary AS
  SELECT l.place_id,
    GROUP_CONCAT(l.name SEPARATOR ', ') AS label_names
  FROM label l
  GROUP BY l.place_id;
...
CREATE VIEW _place_event_summary AS
  SELECT pe.id,
    ...
    fd.end_sun_azimuth,
    COALESCE(ls.label_names, '') AS label_names,
